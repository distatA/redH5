  ...
  <div class="personConfirm">
    ...
    <van-form @submit="onSubmit" >
      ...
      <van-field
        ...
        :rules="[{ required: true, message: '请填写必填项' }]"
      />
      <van-field label-width="130px" name="license_url"  label="企业营业执照">
        <template #input>
          <van-uploader :max-count="1" :after-read="afterChange"  v-model="license_url" />
        </template>
      </van-field>
      <van-field label-width="130px" name="card" label="上传证件照(法人)">
        <template #input> 
...
export default {
  ...
  methods: {
    ...
    async getMes(){
      if(this.type==1){
        const params = {
         ...
         // member_no:'D1F0785164534A1A9D9675EA1872CFFE',
          type:2
        }
        const res = await this.$api.my.details(params)
        console.log('res',res);
        const {name,social_code,mobile,company_name,front_card,reverse_card,province,city,license_url,id} = res.data
        this.name = name
    ...
    async afterChange(file){
      ...
      const res = await apiData.upLoad(params);
      let head = "https://firsthouse.oss-cn-shanghai.aliyuncs.com/";
      const length = this.license_url.length - 1
      console.log(length);
      console.log(this.license_url);
      
      this.license_url[length].url = `${head + res.data}`
    },
    /**
     * 删除之前
     */
    ...
    async onSubmit(values) {
      ...
      if(values.card.length<2){
        ...
        //debugger;
        const image = values.license_url[0].url
        if(this.id){values.id = this.id} 
        values.license_url = image
        ...
        const res = await apiData.my.saveComAuth(values)
        console.log(res);     
        if(res.code == 200){
